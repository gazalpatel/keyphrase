...
class KeyProcesser:
    ...
    def process_text(self, texts, deselect_stop_words=[],
        ...
        tc = TextCleaner(self.text_case, deselect_stop_words, accented_chars, contractions, text_to_number, lemmatization,
                         remove_extra_whitespace, remove_punctuations, remove_html, remove_numbers,
                         remove_special_characters,
                         remove_stop_words, remove_url, para_delim, sentence_delim, sentence_tokenizer)

        if para_delim:
            new_text = tc.para_preprocessing(texts)
        else:
            new_text = tc.text_preprocessing(texts)
